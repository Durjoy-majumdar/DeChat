...
public class WXMusicVideoObject implements WXMediaMessage.IMediaObject {
    ...
    public boolean checkArgs() {
        String str;
        if (C81058b.m99052b(this.musicUrl) || this.musicUrl.length() > 10240) {
            str = "musicUrl.length exceeds the limit";
        } else if (C81058b.m99052b(this.musicDataUrl) || this.musicDataUrl.length() > 10240) {
            str = "musicDataUrl.length exceeds the limit";
        } else if (C81058b.m99052b(this.singerName) || this.singerName.length() > 1024) {
            str = "singerName.length exceeds the limit";
        } else if (!C81058b.m99052b(this.songLyric) && this.songLyric.length() > 32768) {
            str = "songLyric.length exceeds the limit";
        } else if (!C81058b.m99052b(this.hdAlbumThumbFilePath) && this.hdAlbumThumbFilePath.length() > 1024) {
            str = "hdAlbumThumbFilePath.length exceeds the limit";
        } else if (!C81058b.m99052b(this.hdAlbumThumbFilePath) && getFileSize(this.hdAlbumThumbFilePath) > HD_ALBUM_FILE_LENGTH) {
            str = "hdAlbumThumbFilePath file length exceeds the limit";
        } else if (!C81058b.m99052b(this.musicGenre) && this.musicGenre.length() > 1024) {
            str = "musicGenre.length exceeds the limit";
        } else if (!C81058b.m99052b(this.identification) && this.identification.length() > 1024) {
            str = "identification.length exceeds the limit";
        } else if (C81058b.m99052b(this.musicOperationUrl) || this.musicOperationUrl.length() <= 10240) {
            return true;
        } else {
    ...
    public void unserialize(Bundle bundle) {
        ...
        if (string != null) {
            try {
                ...
                Log.m99044e(TAG, "get WXSceneDataObject from bundle failed: unknown vipInfoObjectStr " + string + ", ex = " + e.getMessage());
            }
        }
