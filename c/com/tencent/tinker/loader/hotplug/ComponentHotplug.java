    ...
    public static synchronized void ensureComponentHotplugInstalled(TinkerApplication tinkerApplication) {
        synchronized (ComponentHotplug.class) {
            if (sInstalled) {
                ...
                ShareTinkerLog.m106532i(TAG, "method install() is not invoked, ignore ensuring operations.", new Object[0]);
            }
        }
    }
            ...
            if (!sInstalled) {
                try {
                    if (IncrementComponentManager.init(tinkerApplication, shareSecurityCheck)) {
                        ...
                        sInstalled = true;
                        ShareTinkerLog.m106532i(TAG, "installed successfully.", new Object[0]);
                    }
                } catch (Throwable th) {
                    uninstall();
                    throw new UnsupportedEnvironmentException(th);
        ...
        synchronized (ComponentHotplug.class) {
            if (sInstalled) {
                try {
                    ...
                    ShareTinkerLog.m106531e(TAG, "exception when uninstall.", th);
                }
                sInstalled = false;
            }
        }
